...
export default function FeedPage() {
  ...
  if (error && items.length === 0) {
    return (
      <div className="w-full h-full flex flex-col items-center justify-center text-foreground gap-4">
        <div className="text-red-500 font-semibold">Erreur</div>
        <p className="text-muted-foreground text-sm max-w-xs text-center">{error}</p>
        <button
          onClick={() => refetch()}
          className="px-4 py-2 rounded-full bg-orange-500/20 text-orange-600 border border-orange-500/30 text-sm font-medium hover:bg-orange-500/30 transition-colors"
        >
          Réessayer
        </button>
      </div>
    ...
    return (
      <div className="w-full h-full flex items-center justify-center text-foreground">
        <div className="text-center">
          ...
          <button
            onClick={() => refetch()}
            className="mt-4 px-4 py-2 rounded-full bg-orange-500/20 text-orange-600 border border-orange-500/30 text-sm font-medium hover:bg-orange-500/30 transition-colors"
          >
            Rafraîchir
          </button>
        </div>
  ...
  return (
    <div className="relative w-full h-full overflow-hidden">
      {/* Indicateur de progression Streak (Objectif 5 posts) */}
      <div className="absolute top-4 left-0 right-0 z-20 px-6 flex flex-col items-center gap-2 pointer-events-none">
        <div className="flex items-center gap-1 w-full max-w-[120px]">
          {[0, 1, 2, 3, 4].map((index) => {
            ...
            return (
              <div
                key={index}
                className="h-0.5 flex-1 bg-white/10 rounded-full overflow-hidden backdrop-blur-sm"
              >
                <motion.div
                  initial={false}
        ...
        <AnimatePresence>
          {showStreakNotification && (
            <motion.button
              ...
              className="flex items-center gap-1.5 bg-orange-500/90 backdrop-blur-md px-3 py-1 rounded-full shadow-lg pointer-events-auto"
            >
      ...
      <AnimatePresence initial={false} custom={exitDirection} mode="popLayout">
        ...
        >
          ...
          {/* Overlay indicateur swipe vers le haut (détails) */}
          <motion.div
            className="absolute inset-0 bg-gradient-to-b from-transparent to-purple-500/20 pointer-events-none flex items-end justify-center pb-24"
                ...
                <div className="space-y-3">
                  <div className="space-y-1">
                    <div className="flex justify-between text-sm">
                      ...
                      <span className="text-blue-500">{currentItem.realGauge}%</span>
                    </div>
                  ...
                  <div className="space-y-1">
                    <div className="flex justify-between text-sm">
                      ...
                      <span className="text-orange-500">{currentItem.conceptualGauge}%</span>
                    </div>
                    <div className="h-2 bg-muted rounded-full overflow-hidden">
                      <motion.div
                        initial={{ width: 0 }}
